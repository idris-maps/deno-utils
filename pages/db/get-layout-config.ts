import { getLayoutConfig, LayoutConfig, parseYaml } from "../deps.ts";
...
const checkFileExists = async (filename: string) => {
  try {
    ...
    throw new Error(
      `[layout config] could not read ${filename}: ${err.message}`,
    );
  }
};
...
const parseAsJson = (file: string) => {
  try {
    ...
    throw new Error(
      `[layout config] could not read file as json: ${err.message}`,
    );
  }
};
...
const parseAsYaml = (file: string) => {
  try {
    ...
    throw new Error(
      `[layout config] could not read file as yaml: ${err.message}`,
    );
  }
};

export const readLayoutConfig = async (
  filename?: string,
