...
import { throwValidationError, throwValidationErrors, getValidationErrors } from './validator.ts'
    ...
    if (!Array.isArray(d)) {
      ...
      const tests = d.map((d, i) => {
        try {
          ...
          getValidationErrors(err).forEach(d => errors.push(({ ...d, path: `[${i}]${d.path ? '.' + d.path : ''}` })))
          return false;
        }
      })
