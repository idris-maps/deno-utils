...
const fitsPattern = (pattern: string, d: string) =>
  ...
  return (d: unknown): d is string => {
    if (isString(d) && props) {
      if (props.maxLength && d.length > props.maxLength) {
        throwValidationError({ expected: `maxLength ${props.maxLength}`, got: d })
      }
      if (props.minLength && d.length > props.minLength) {
        throwValidationError({ expected: `minLength ${props.minLength}`, got: d })
      }
